...
import { useTripStore, useAuthStore, useSubscriptionStore } from '../store';
...
<div class="cover">
  <div class="cover-brand">Roamly · Your trip, perfectly mapped.</div>
...
<div class="footer">
  Generated by <strong>Roamly</strong> · roamly.app · AI-curated, human-approved
</div>
...
export default function ItineraryScreen() {
  ...
  const [activeDay,   setActiveDay]   = useState(-1); // -1 = Highlights tab (first)
  ...
  const heroShimmerAnim = useRef(new Animated.Value(0.3)).current;

  const tripIsPast = currentItinerary
    ? (currentItinerary.tripInput.endDate instanceof Date ? currentItinerary.tripInput.endDate : new Date(currentItinerary.tripInput.endDate)).getTime() < Date.now()
    : false;
  const activeDayWeather = weather[activeDay];
  const activeDayBad     = !tripIsPast && activeDayWeather ? isBadWeather(activeDayWeather) : false;
  // -1 = activity days, days.length = dining tab
  // Tab indices: Highlights(0) → Day1(1) → Day2(2) → ... → Packing → Dining
  const HIGHLIGHTS_TAB = -1;
  const numDays        = currentItinerary?.days.length ?? 0;
  const PACKING_TAB    = numDays;
  const DINING_TAB     = numDays + 1;
  ...
  // ── Celebration on itinerary reveal (first load only) ───────────────────
  const hasRevealedRef = useRef(false);
  useEffect(() => {
    if (currentItinerary && !hasRevealedRef.current) {
  ...
  useEffect(() => {
    if (!heroImage) {
      const loop = Animated.loop(
        Animated.sequence([
          Animated.timing(heroShimmerAnim, { toValue: 0.6, duration: 800, useNativeDriver: true }),
          Animated.timing(heroShimmerAnim, { toValue: 0.3, duration: 800, useNativeDriver: true }),
        ])
      );
      loop.start();
      return () => loop.stop();
  ...
  // ── Unsaved itinerary back warning ─────────────────────────────────────
  useEffect(() => {
    if (!currentItinerary) return;
    const onBack = () => {
      if (!isSaved && !isGuest) {
        Alert.alert(
          'Save before leaving?',
          'Your itinerary will be lost if you go back without saving.',
          [
            { text: 'Save & Go Back', onPress: async () => { await handleSaveTrip(); router.back(); } },
    ...
    return () => sub.remove();
  }, [currentItinerary, isSaved, isGuest]);
  ...
  const handleShare = async () => {
    ...
    const message =
      ...
      `${dayLines}\n\n` +
      `✨ Highlights\n${highlightLines}\n\n` +
      `Created with Roamly — Your trip, perfectly mapped.\n\nDownload: https://play.google.com/store/apps/details?id=com.familyquest.app`;

    try {
      await Share.share({ message, title: `${destination} Trip — Roamly` });
    } catch (_) {}
  };
  ...
  const handleSaveTrip = async () => {
    ...
    if (isSaved) return;
    if (savedTrips.length >= 3 && !isPremium) { setShowUpgrade(true); return; }
  ...
  const handleRegenerate = async () => {
    if (!currentItinerary) return;
    if (regenCount > 0 && !isPremium) { setShowUpgrade(true); return; }
    Alert.alert(
    ...
    return (
      <View style={styles.empty}>
        <Svg width={80} height={80} viewBox="0 0 80 80">
          <Circle cx="40" cy="40" r="36" stroke="#F05A28" strokeWidth="3" fill="none" />
          <Circle cx="40" cy="40" r="28" stroke="#5B8DEF" strokeWidth="1.5" fill="none" strokeDasharray="4 4" />
          <Path d="M40 12 L44 40 L40 68" stroke="#F05A28" strokeWidth="2.5" fill="none" strokeLinecap="round" />
          <Path d="M12 40 L40 36 L68 40" stroke="#5B8DEF" strokeWidth="2.5" fill="none" strokeLinecap="round" />
          <Circle cx="40" cy="40" r="4" fill="#F05A28" />
          <Line x1="54" y1="54" x2="62" y2="62" stroke="#5B8DEF" strokeWidth="2" strokeDasharray="3 3" strokeLinecap="round" />
          <Circle cx="65" cy="65" r="3" fill="#5B8DEF" />
        </Svg>
  ...
  return (
    <View style={styles.container}>
      ...
      <Animated.View style={[styles.heroHeader, { height: headerHeight }]}>
        {/* Unsplash photo or shimmer placeholder */}
        {heroImage ? (
          <Image source={{ uri: heroImage }} style={styles.heroImage} resizeMode="cover" />
        ) : (
          <Animated.View style={[StyleSheet.absoluteFill, { backgroundColor: 'rgba(61,31,110,0.40)', opacity: heroShimmerAnim }]} />
        )}
      ...
      >
        ...
        <View style={styles.dayTabsWrap}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.dayTabs}>
            {/* Highlights — first tab */}
            <TouchableOpacity
              onPress={() => setActiveDay(HIGHLIGHTS_TAB)}
              style={[styles.dayTab, activeDay === HIGHLIGHTS_TAB && styles.dayTabActive, styles.dayTabHighlights]}
              activeOpacity={0.8}
            >
              <Text style={[styles.dayTabNum, activeDay === HIGHLIGHTS_TAB && styles.dayTabTextActive]}>✨</Text>
              <Text style={[styles.dayTabDate, activeDay === HIGHLIGHTS_TAB && { color: 'rgba(255,255,255,0.65)' }]}>Highlights</Text>
            </TouchableOpacity>
            {/* Day tabs */}
            {days.map((day, i) => {
              const tripStart = currentItinerary.tripInput.startDate;
              const tabDate = new Date(tripStart instanceof Date ? tripStart : new Date(tripStart));
              tabDate.setDate(tabDate.getDate() + i);
              const w = !tripIsPast ? weather[i] : undefined;
              const rainy = w ? isBadWeather(w) : false;
              return (
                <TouchableOpacity
                  key={i}
                  onPress={() => setActiveDay(i)}
                  style={[styles.dayTab, activeDay === i && styles.dayTabActive, rainy && !tripIsPast && styles.dayTabRainy]}
                  activeOpacity={0.8}
                >
            ...
            {/* Packing — before dining */}
            <TouchableOpacity
              onPress={() => setActiveDay(PACKING_TAB)}
              style={[styles.dayTab, activeDay === PACKING_TAB && styles.dayTabActive, styles.dayTabPacking]}
              activeOpacity={0.8}
            >
              <Text style={[styles.dayTabNum, activeDay === PACKING_TAB && styles.dayTabTextActive]}>🎒</Text>
              <Text style={[styles.dayTabDate, activeDay === PACKING_TAB && { color: 'rgba(255,255,255,0.65)' }]}>Packing</Text>
            </TouchableOpacity>
            {/* Dining — always last */}
            <TouchableOpacity
        ...
        {isGuest && !bannerDismissed && (
          <View style={styles.guestBanner}>
            <View style={{ flex: 1 }}>
              <Text style={styles.guestBannerTitle}>🔐 Sign up to keep your trips forever</Text>
              <TouchableOpacity
            ...
            <TouchableOpacity onPress={() => setBannerDismissed(true)} style={styles.guestBannerClose}>
              <Text style={styles.guestBannerCloseText}>✕</Text>
            </TouchableOpacity>
          </View>
        )}

        {/* Bad weather toggle — in scroll area so it's always tappable */}
        {activeDayBad && activeDay >= 0 && activeDay < numDays && (
          <TouchableOpacity
            style={[styles.weatherToggleBar, showIndoorPlan && styles.weatherToggleBarOn]}
            onPress={() => setShowIndoorPlan(v => !v)}
            activeOpacity={0.82}
          >
            <Text style={styles.weatherToggleBarIcon}>{showIndoorPlan ? '🏠' : '⛈️'}</Text>
            <View style={{ flex: 1 }}>
              <Text style={styles.weatherToggleBarTitle}>
                {showIndoorPlan ? 'Indoor plan active' : 'Rain expected today'}
              </Text>
              <Text style={styles.weatherToggleBarSub}>
                {showIndoorPlan ? 'Showing indoor alternatives · Tap to switch back' : 'Tap to switch to indoor alternatives'}
              </Text>
            </View>
            <View style={[styles.weatherTogglePill, showIndoorPlan && styles.weatherTogglePillOn]}>
              <Text style={styles.weatherTogglePillText}>{showIndoorPlan ? 'ON' : 'OFF'}</Text>
            </View>
          </TouchableOpacity>
        )}

        {/* Active day */}
        {activeDay >= 0 && activeDay < numDays && days[activeDay] && (
          <Animated.View style={[styles.daySection, { opacity: fadeAnim }]}>
            <SlideUp key={activeDay}>
              ...
              {/* ── Weather card — hidden for past trips ── */}
              {!tripIsPast && activeDayWeather ? (
                <WeatherCard weather={activeDayWeather} />
              ) : !tripIsPast && weatherError === 'too_far' ? (
              ...
              ) : !tripIsPast && weatherError === 'failed' ? (
                <View style={styles.weatherNoData}>
        ...
        {/* Highlights tab */}
        {activeDay === HIGHLIGHTS_TAB && (
          <View style={styles.daySection}>
            <SlideUp key="highlights">
              <View style={styles.dayThemeRow}>
                <View style={styles.dayThemeBadge}>
                  <Text style={styles.dayThemeText}>Trip highlights</Text>
                </View>
                <Text style={styles.activityCount}>{highlights.length} highlights</Text>
              </View>
        ...
        {/* Packing tab */}
        {activeDay === PACKING_TAB && (
          <View style={styles.daySection}>
            <SlideUp key="packing">
              <View style={styles.dayThemeRow}>
                <View style={styles.dayThemeBadge}>
                  <Text style={styles.dayThemeText}>What to pack</Text>
                </View>
                <Text style={styles.activityCount}>{packingTips.length} items</Text>
              </View>
              <GlassCard style={styles.infoCard}>
                {packingTips.map((t, i) => (
                  <View key={i} style={styles.infoRow}>
      ...
      <View style={styles.topActions} pointerEvents="box-none">
        <TouchableOpacity onPress={() => router.back()} style={styles.actionBtn}>
          <Text style={[styles.actionBtnText, { fontSize: 22 }]}>◂</Text>
        </TouchableOpacity>
        <View />
      </View>

      {/* Celebration reveal overlay */}
      {showReveal && currentItinerary && (
        <Animated.View style={[styles.revealOverlay, { opacity: revealOpacity }]}>
          <ConfettiBurst trigger={1} />
          <Text style={styles.revealTitle}>Your trip is ready! 🎉</Text>
          <Text style={styles.revealDest}>{currentItinerary.destination}</Text>
        </Animated.View>
      )}
      ...
      <View style={[styles.activityBody, { borderLeftColor: indoorActive ? '#2563EB' : meta.color }]}>
        <View style={styles.activityHeader}>
          ...
          <View style={{ flex: 1, minWidth: 0 }}>
            <Text style={styles.activityTitle} numberOfLines={indoorActive ? 1 : undefined}>
              {indoorActive ? activity.indoorAlternative : activity.title}
            </Text>
            {indoorActive && (
              <Text style={styles.indoorOriginal} numberOfLines={1}>Replaces: {activity.title}</Text>
            )}
...
const styles = StyleSheet.create({
  ...
  fabGradient: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 16, paddingHorizontal: 20, gap: 8 },
  fabIcon:  { fontSize: 18 },
  ...
  dayTabRainy:   { borderColor: '#5B8DEF', backgroundColor: 'rgba(91,141,239,0.12)', borderWidth: 2 },
  dayTabHighlights: { borderColor: 'rgba(245,166,35,0.35)', backgroundColor: 'rgba(245,166,35,0.06)' },
  dayTabPacking:    { borderColor: 'rgba(67,201,122,0.35)', backgroundColor: 'rgba(67,201,122,0.06)' },
  ...
  weatherAlertBadgeText: { fontSize: Typography.xs, fontFamily: Typography.extraBold, letterSpacing: 0.3 },
  weatherCardAdvice: {
  ...
  // ── Weather toggle bar (in scroll body) ────────────────────────────────────
  weatherToggleBar: {
  ...
  weatherToggleBarIcon: { fontSize: 24 },
  weatherToggleBarTitle: { fontSize: Typography.sm, fontFamily: Typography.bold, color: '#1E40AF' },
  weatherToggleBarSub: { fontSize: Typography.xs, fontFamily: Typography.regular, color: '#3B82F6', marginTop: 1 },
  weatherTogglePill: {
    paddingHorizontal: 12, paddingVertical: 5, borderRadius: Radius.full,
    backgroundColor: 'rgba(59,130,246,0.15)', borderWidth: 1, borderColor: 'rgba(59,130,246,0.30)',
  },
  weatherTogglePillOn: {
    backgroundColor: '#3B82F6', borderColor: '#2563EB',
  },
  weatherTogglePillText: { fontSize: Typography.xs, fontFamily: Typography.extraBold, color: '#1E40AF' },

  // ── Hero weather toggle (legacy) ──────────────────────────────────────────
  heroWeatherToggle: {
    flexDirection: 'row', alignItems: 'center', gap: 8,
    paddingVertical: 8, paddingHorizontal: 16,
    borderRadius: Radius.full, marginBottom: 8,
    backgroundColor: 'rgba(91,141,239,0.25)',
    borderWidth: 1.5, borderColor: 'rgba(91,141,239,0.50)',
  },
  heroWeatherToggleOn: {
    backgroundColor: 'rgba(37,99,235,0.35)',
    borderColor: 'rgba(37,99,235,0.70)',
  },
  heroWeatherToggleIcon: { fontSize: 16 },
  heroWeatherToggleText: { fontSize: Typography.xs, fontFamily: Typography.bold, color: '#fff' },
  ...
  guestBanner: {
    ...
    borderWidth: 1.5, borderColor: 'rgba(139,92,246,0.22)',
  },
  guestBannerTitle: {
    fontSize: Typography.sm, fontFamily: Typography.bold,
    color: Colors.info, marginBottom: 8,
  },
  guestBannerBtn: {
  ...
  activityCardIndoor: { borderColor: 'rgba(59,130,246,0.30)', backgroundColor: 'rgba(59,130,246,0.05)' },
  indoorActiveBanner: {
  ...
  indoorOriginal: { fontSize: Typography.xs, fontFamily: Typography.regular, color: Colors.textLight, marginTop: 2, textDecorationLine: 'line-through' },
  ...
  // ── Reveal celebration overlay ────────────────────────────────────────
  revealOverlay: {
    ...StyleSheet.absoluteFillObject,
    zIndex: 100, backgroundColor: 'rgba(13,5,32,0.85)',
    alignItems: 'center', justifyContent: 'center',
  },
  revealTitle: {
    fontSize: Typography['2xl'], fontFamily: Typography.black, color: '#fff',
    textAlign: 'center', marginBottom: 8,
  },
  revealDest: {
    fontSize: Typography['3xl'], fontFamily: Typography.black, color: Colors.gold,
    textAlign: 'center',
  },
});
